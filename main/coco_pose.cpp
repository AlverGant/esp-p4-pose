...
Yolo11nPose::Yolo11nPose(const char *model_name)
{
    ...
    // Optimized thresholds for better pose detection:
    // - Score: 0.25 → 0.20 (more sensitive, detects partial/difficult poses)
    // - NMS: 0.7 → 0.65 (less aggressive filtering, keeps more candidates)
    // - Top_k: 10 → 15 (detect more people simultaneously)
    m_postprocessor = new dl::detect::yolo11posePostProcessor(
        m_model, m_image_preprocessor, 0.20, 0.65, 15, {{8, 8, 4, 4}, {16, 16, 8, 8}, {32, 32, 16, 16}});
}
